...
```mermaid
flowchart TD
    subgraph HW["🔴 Hardware — MAX30102"]
        SENSOR["MAX30102\n(LED Red 660nm + IR 880nm)"]
        FIFO["FIFO Interna\n(32 amostras × 6 bytes)"]
        I2C["Barramento I2C\n(400 kHz, SDA=22, SCL=23)"]
        SENSOR --> FIFO --> I2C
    end

    subgraph MCU["⚙️ ESP32-C6 (160 MHz, RISC-V)"]
        direction TB

        subgraph DRAIN["Estágio 1 — Drenagem da FIFO"]
            READ["Leitura burst\n(6 bytes/amostra)"]
            PARSE["Parsing 18-bit\nRed: bytes 0-2\nIR: bytes 3-5"]
            READ --> PARSE
        end

        subgraph FINGER["Detecção de Dedo"]
            FDET["IR_raw > baseline + 9000\n(histerese 3 amostras)"]
        end

        subgraph FILT["Estágio 2 — Condicionamento de Sinal (×2 canais)"]
            DC_EST["Estimador DC\nEMA α=0.1→0.005\n(settling 300 amostras)"]
            DC_SUB["Subtração DC\nac = raw − dc"]
            LPF["Butterworth LPF\n2ª ordem, fc=5 Hz\nDirect Form II"]
            DC_EST --> DC_SUB --> LPF
        end

        subgraph HR_DET["Estágio 3 — Frequência Cardíaca"]
            DERIV["Derivada span=4\nd[n] = x[n] − x[n−4]"]
            ZC["Zero-crossing\nd: +→− (pico)"]
            AMP["Gate de amplitude\namp > 30% × threshold\namp > 100 (mínimo)"]
            REFR["Período refratário\n400 ms (max 150 BPM)"]
            THRESH["Threshold adaptativo\n0.92·thr + 0.08·amp\n(cap 2×, decay 0.998/s)"]
            MED_HR["Mediana de 8\nintervalos → BPM"]
            DERIV --> ZC --> AMP --> REFR --> THRESH --> MED_HR
        end

        subgraph SPO2["Estágio 4 — SpO₂"]
            ACCUM["Acumular AC/DC\npor batimento\n(min/max IR, Red)"]
            RATIO["R = (AC_Red/DC_Red)\n÷ (AC_IR/DC_IR)"]
            GATE["Gate de qualidade\n4 critérios:\nlength, AC>50, PI>0.05%, R∈[0.2,1.8]"]
            MED_R["Mediana de 4 R\n→ SpO₂ = f(R)"]
            CALIB["Calibração AN6409\nSpO₂ = −45.06R² + 30.35R + 94.85"]
            ACCUM --> RATIO --> GATE --> MED_R --> CALIB
        end
    end

    subgraph OUT["📊 Saída"]
        BPM["Frequência Cardíaca\n(BPM)"]
        SPO2_OUT["Saturação O₂\n(SpO₂ %)"]
    end

    I2C --> READ
    PARSE --> FDET
    PARSE -->|"IR_raw, Red_raw\n(100 Hz)"| DC_EST
    LPF -->|"IR_ac"| DERIV
    LPF -->|"IR_ac, Red_ac"| ACCUM
    DC_EST -->|"IR_dc, Red_dc"| ACCUM
    MED_HR -->|"trigger beat"| ACCUM
    MED_HR --> BPM
    CALIB --> SPO2_OUT
```

**Figura 1** — Diagrama completo do pipeline de processamento PPG implementado. O fluxo inicia no sensor MAX30102 (aquisição óptica e conversão ADC de 18 bits), atravessa o barramento I2C até o ESP32-C6, onde passa por quatro estágios de processamento digital: drenagem e parsing da FIFO, condicionamento de sinal (remoção DC + LPF Butterworth), detecção de picos para frequência cardíaca, e extração de SpO₂ por razão das razões com gate de qualidade. A detecção de presença do dedo opera em paralelo sobre o sinal bruto.

A representação em texto ASCII abaixo serve como referência alternativa para ambientes que não suportam Mermaid:
